...
int aes_cbc_encrypt_init(AES_CTX *c, const uint8 key[16])
{
	if (c == NULL || key == NULL)
		return -1;
	...
	NPI_Printf("aes_cbc_encrypt_init\r\n");
...
int aes_cbc_encrypt_update(AES_CTX *c, const uint8 in[16], uint8 out[16])
{
...
int aes_cbc_encrypt_final(AES_CTX *c, const uint8 *in, int in_len, uint8 out[32], int *out_len)
{
...
int aes_cbc_decrypt_init(AES_CTX *c, const uint8 key[16])
{
	if (c == NULL || key == NULL)
		return -1;
...
int aes_cbc_decrypt_update(AES_CTX *c, const uint8 in[16], uint8 out[16])
{
	if (c == NULL || in == NULL || out == NULL)
		return -1;
...
int aes_cbc_decrypt_final(AES_CTX *c, const uint8 in[16], uint8 out[16], int *out_len)
{
	if (c == NULL || in == NULL || out == NULL)
		return -1;
